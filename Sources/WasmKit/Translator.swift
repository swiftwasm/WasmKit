...
struct InstructionTranslator<Context: TranslatorContext>: InstructionVisitor {
    ...
    mutating func visitIf(blockType: WasmParser.BlockType) throws -> Output {
        ...
        iseqBuilder.emitWithLabel(endLabel) { iseqBuilder, endPC in
            ...
            return .ifThen(Instruction.IfOperand(
                elseOrEndOffset: elseOrEnd, condition: condition
            ))
        }
    }
    ...
    private mutating func visitBinary(
        ...
        if let lhs = lhs, let rhs = rhs {
            ...
            case (.vreg(let lhs), .vreg(let rhs)):
                preservePReg(.r0)
                instruction = commutative.ss(lhs, rhs)
            case (.vreg(let vreg), .preg), (.preg, .vreg(let vreg)):
                instruction = commutative.sr(vreg)
            case (.preg, .preg):
                fatalError()
            }
            emit(instruction)
    ...
    private mutating func visitBinary(
        ...
        if let lhs = lhs, let rhs = rhs {
            ...
            case (.vreg(let lhs), .vreg(let rhs)):
                preservePReg(.r0)
                instruction = nonCommutative.ss(lhs, rhs)
            case (.vreg(let vreg), .preg):
                instruction = nonCommutative.sr(vreg)
            case (.preg, .vreg(let vreg)):
                instruction = nonCommutative.rs(vreg)
            case (.preg, .preg):
                fatalError()
            }
            emit(instruction)
