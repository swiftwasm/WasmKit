...
/// A point in time after which `parkConditionally` gives up waiting.
struct ParkingDeadline {
    #if MultiThread
        let instant: ContinuousClock.Instant
    #endif

    init(afterNanoseconds nanoseconds: Int64) {
        #if MultiThread
            self.instant = ContinuousClock.now.advanced(by: .nanoseconds(nanoseconds))
        #endif
    }
}

// The blocking implementation requires OS threads. Single-threaded builds
// (MultiThread trait disabled) cannot create shared memories, so
// `atomic.wait`/`notify` trap before reaching the parking lot; the stub below
// only satisfies the type checker.
    ...
    package final class AtomicParkingLot: Sendable {
        ...
        func parkConditionally(
            ...
            if let deadline = deadline {
                let remaining = deadline.instant - ContinuousClock.now
    ...
    package final class AtomicParkingLot: Sendable {
        ...
        func unpark(address: UInt64, count: UInt32) -> UInt32 { 0 }
