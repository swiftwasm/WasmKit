    ...
    public func invoke(_ arguments: [ComponentValue] = []) throws -> [ComponentValue] {
        ...
        if let resultType = handle.type.result {
            ...
            if flatCount > MAX_FLAT_RESULTS {
                ...
                results = try [liftValueFromMemory(
                    at: retptr,
                    type: resultType,
                    resolveType: handle.resolveType,
                    options: handle.canonOptions,
...
// MARK: - Canonical ABI Value Lowering/Lifting
...
func lowerValue(
    ...
    case (.tuple(let elements), .tuple(let typeIndices)):
        ...
        for (element, typeIndex) in zip(elements, typeIndices) {
            ...
            let lowered = try lowerValue(element, to: elementType, resolveType: resolveType, options: options, store: store)
    ...
    case (.option(let maybeValue), .option(let someTypeIndex)):
        ...
        if let value = maybeValue {
            ...
            let lowered = try lowerValue(value, to: someType, resolveType: resolveType, options: options, store: store)
            results.append(contentsOf: lowered)
            return results
        } else {
    ...
    case (.record(let fields), .record(let fieldDefs)):
        ...
        for fieldDef in fieldDefs {
            ...
            let lowered = try lowerValue(fieldValue, to: fieldType, resolveType: resolveType, options: options, store: store)
            results.append(contentsOf: lowered)
        }
        return results
    ...
    case (.result(let ok, let error), .result(let okTypeIndex, let errorTypeIndex)):
        ...
        if let okValue = ok {
            ...
            if let okType = okType {
                let okLowered = try lowerValue(okValue, to: okType, resolveType: resolveType, options: options, store: store)
            ...
            if let errorType = errorType {
                let errorLowered = try lowerValue(errorValue, to: errorType, resolveType: resolveType, options: options, store: store)
    ...
    case (.variant(let caseName, let payload), .variant(let cases)):
        ...
        let maxPayloadCount = try cases.map { variantCase -> Int in
        ...
        if let typeIndex = cases[caseIndex].type, let payloadValue = payload {
            ...
            let caseLowered = try lowerValue(payloadValue, to: caseType, resolveType: resolveType, options: options, store: store)
    ...
    default:
        throw CanonicalABIError(
            description: "Cannot lower \(value) to \(type)"
        )
    }
}
...
private func sizeOf(
    ...
    case .variant(let cases):
        let maxPayloadSize = try cases.map { variantCase -> Int in
            if let typeIndex = variantCase.type {
...
private func alignmentOf(
    ...
    case .variant(let cases):
        let maxPayloadAlign = try cases.compactMap { variantCase -> Int? in
...
private func storeValue(
    ...
    case (.variant(let caseName, let payload), .variant(let cases)):
        ...
        if let typeIndex = cases[caseIndex].type, let payloadValue = payload {
            ...
            try storeValue(payloadValue, type: caseType, at: payloadOffset, memory: memory, resolveType: resolveType, options: options, store: store)
        }
...
func liftValue(
    ...
    case .tuple(let typeIndices):
        ...
        for typeIndex in typeIndices {
            let elementType = try resolveType(typeIndex)
            let element = try liftValue(from: &iterator, to: elementType, resolveType: resolveType, options: options, store: store)
    ...
    case .option(let someTypeIndex):
        ...
        if discriminant == 0 {
            ...
            // some: lift the value
            let value = try liftValue(from: &iterator, to: someType, resolveType: resolveType, options: options, store: store)
            return .option(value)
        }
    ...
    case .record(let fieldDefs):
        ...
        for fieldDef in fieldDefs {
            let fieldType = try resolveType(fieldDef.type)
            let value = try liftValue(from: &iterator, to: fieldType, resolveType: resolveType, options: options, store: store)
    ...
    case .result(let okTypeIndex, let errorTypeIndex):
        ...
        if discriminant == 0 {
            // ok case
            if let okType = okType {
                let value = try liftValue(from: &iterator, to: okType, resolveType: resolveType, options: options, store: store)
                // Consume padding
            ...
            // error case
            if let errorType = errorType {
                let value = try liftValue(from: &iterator, to: errorType, resolveType: resolveType, options: options, store: store)
                // Consume padding
    ...
    case .variant(let cases):
        ...
        if let typeIndex = variantCase.type {
            // Case with payload
            let caseType = try resolveType(typeIndex)
            let value = try liftValue(from: &iterator, to: caseType, resolveType: resolveType, options: options, store: store)
            // Consume padding
...
private func liftValueFromMemory(
    at offset: UInt32,
    type: ComponentValueType,
    resolveType: (ComponentTypeIndex) throws -> ComponentValueType,
    options: CanonOptions,
    ...
    default:
        throw CanonicalABIError(description: "Cannot lift \(type) from memory - not yet implemented")
    }
}

// MARK: - Legacy Flat ABI Functions (for backward compatibility)
...
extension ComponentValueType {
    ...
    var flattenedComponentValueType: [ValueType] {
        ...
        default:
            ...
            #warning("Complex types not supported yet in `flattenComponentValueType`")
            return []
        }
    }
