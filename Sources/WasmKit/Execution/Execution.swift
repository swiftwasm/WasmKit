...
struct Execution: ~Copyable {
    ...
    /// looks: the images are tiny (2 slots for `fibonacci-rec`'s only function,
    /// ~16 on average across the wasmi-benchmarks suite), so a libc call costs
    /// several times the copy itself. Before this was inlined, `_platform_memmove`
    /// plus its lazy-binding stub took ~20% of `fibonacci-rec`'s samples.
    @inline(__always)
    ...
    /// Copies two slots as one 16-byte unit, so this lowers to a single
    /// `ldr q`/`str q` pair rather than two `ldr`/`str`.
    @inline(__always)
