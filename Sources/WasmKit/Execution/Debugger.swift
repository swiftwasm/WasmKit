...
import WasmParserCore
    ...
    /// debugger protocol, which allows any protocol implementation or direct API users to be layered on top if needed.
        ...
        package enum Error: Swift.Error, @unchecked Sendable {
            ...
            #if $Embedded
            case wasmKitError(WasmKitError)
            case trap(Trap)
            #endif
        }

        #if $Embedded
        private static func wrap<T>(_ body: () throws(WasmKitError) -> T) throws(Debugger.Error) -> T {
            do throws(WasmKitError) { return try body() } catch let e { throw .wasmKitError(e) }
        }
        private static func wrapTrap<T>(_ body: () throws(Trap) -> T) throws(Debugger.Error) -> T {
            do throws(Trap) { return try body() } catch let e { throw .trap(e) }
        }
        #endif
        ...
        package mutating func run() throws {
            do {
                ...
                case .instantiated:
                    ...
                    #else
                    var rootSlot: CodeSlot = Instruction.endOfExecution.headSlot(threadingModel: self.threadingModel)
                    var thrownTrap: Trap? = nil
                    withUnsafeMutablePointer(to: &rootSlot) { rootPtr in
                        do throws(Trap) {
                            try self.execution.executeEmbedded(
                                rootSp: self.valueStack,
                                rootISQ: rootPtr,
                                handle: self.entrypointFunction.handle,
                                type: self.entrypointFunction.type
                            )
                        } catch let trap { thrownTrap = trap }
                    }
                    if let trap = thrownTrap { throw trap }
                    let result: [Value] = []  // embedded doesn't return values via debugger path
                    #endif
        ...
        ///   - store: Store that instantiates the module.
        ///   - imports: Imports required by `module` for instantiation.
        package init(module: Module, store: Store, imports: Imports) throws(Debugger.Error) {
            let limit = store.engine.configuration.stackSize / MemoryLayout<StackSlot>.stride
            let instance = try Debugger.wrap { () throws(WasmKitError) -> Instance in try module.instantiate(store: store, imports: imports, isDebuggable: true) }
        ...
        /// Sets a breakpoint at the first instruction in the entrypoint function of the module instantiated by
        /// this debugger.
        package mutating func stopAtEntrypoint() throws(Debugger.Error) {
            try self.enableBreakpoint(address: self.originalAddress(function: entrypointFunction))
        }
        ...
        /// - Parameter function: the Wasm function to find the first Wasm instruction address for.
        /// - Returns: byte offset of the first Wasm instruction of given function in the module it was parsed from.
        package func originalAddress(function: Function) throws(Debugger.Error) -> Int {
            precondition(function.handle.isWasm)
            ...
            case .debuggable(let wasm, _):
                return wasm.originalAddress
            case .uncompiled:
                try Debugger.wrapTrap { () throws(Trap) in try function.handle.wasm.ensureCompiled(store: StoreRef(self.store)) }
                return try self.originalAddress(function: function)
            case .compiled:
                fatalError()
            }
        ...
        private func findIseq(forWasmAddress address: Int) throws(Debugger.Error) -> (iseq: Pc, wasm: Int) {
            if let (iseq, wasm) = self.instance.handle.instructionMapping.findIseq(forWasmAddress: address) {
                return (iseq, wasm)
            }
            ...
            let functionIndex = self.functionAddresses[followingIndex - 1].instanceFunctionIndex
            let function = instance.handle.functions[functionIndex]
            try Debugger.wrapTrap { () throws(Trap) in try function.wasm.ensureCompiled(store: StoreRef(self.store)) }
        ...
        /// See also ``Debugger/disableBreakpoint(address:)``.
        @discardableResult
        package mutating func enableBreakpoint(address: Int) throws(Debugger.Error) -> Int {
            guard self.breakpoints[address] == nil else {
                return address
            }
        ...
        /// instruction is restored from debugger state and replaces the breakpoint instruction.
        /// See also ``Debugger/enableBreakpoint(address:)``.
        package mutating func disableBreakpoint(address: Int) throws(Debugger.Error) {
            guard let oldCodeSlot = self.breakpoints[address] else {
                return
            }
        ...
        /// and execution is resumed until the next breakpoint is triggered or all remaining instructions are
        /// executed. If the module is not stopped at a breakpoint, this function returns immediately.
        package mutating func run() throws(Debugger.Error) {
            do {
                switch self.state {
                case .stoppedAtBreakpoint(let breakpoint):
                    // Remove the breakpoint before resuming
        ...
        /// of multiple possible execution branches). After breakpoints setup, execution is resumed until suspension.
        /// If the module is not stopped at a breakpoint, this function returns immediately.
        package mutating func step() throws(Debugger.Error) {
            guard case .stoppedAtBreakpoint(let breakpoint) = self.state else {
                return
            }
        ...
        /// the debugger resumes is preserved. If the module is current not stopped at a breakpoint, this function
        /// returns immediately.
        package mutating func runPreservingCurrentBreakpoint() throws(Debugger.Error) {
            guard case .stoppedAtBreakpoint(let breakpoint) = self.state else {
                return
            }
        ...
        package func getLocal(frameIndex: UInt, localIndex: UInt) throws(Debugger.Error) -> UInt64 {
            guard case .stoppedAtBreakpoint(let breakpoint) = self.state else {
                throw Error.notStoppedAtBreakpoint
            }
        ...
        /// Analyzes the control-flow instruction at the given breakpoint and sets breakpoints
        /// at all possible next instruction locations.
        private mutating func setNextInstructionBreakpoints(breakpoint: BreakpointState) throws(Debugger.Error) {
            // If the breakpoint was externally removed (e.g. via disableBreakpoint
            // while stopped), the original instruction has already been restored at the
            // iseq PC, so read it directly.
            let savedHead = self.breakpoints[breakpoint.wasmPc] ?? breakpoint.iseq.pc.pointee
