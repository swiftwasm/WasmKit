...
extension ExecutionState {
    @inline(__always)
    mutating func doExecute(_ instruction: Instruction, currentMemory: CurrentMemory, runtime: Runtime, stack: inout Stack) throws -> Bool {
        switch instruction {
        ...
        case .i32Load(let loadOperand):
            try self.i32Load(runtime: runtime, stack: &stack, currentMemory: currentMemory, loadOperand: loadOperand)
        case .i64Load(let loadOperand):
            try self.i64Load(runtime: runtime, stack: &stack, currentMemory: currentMemory, loadOperand: loadOperand)
        case .f32Load(let loadOperand):
            try self.f32Load(runtime: runtime, stack: &stack, currentMemory: currentMemory, loadOperand: loadOperand)
        case .f64Load(let loadOperand):
            try self.f64Load(runtime: runtime, stack: &stack, currentMemory: currentMemory, loadOperand: loadOperand)
        case .i32Load8S(let loadOperand):
            try self.i32Load8S(runtime: runtime, stack: &stack, currentMemory: currentMemory, loadOperand: loadOperand)
        case .i32Load8U(let loadOperand):
            try self.i32Load8U(runtime: runtime, stack: &stack, currentMemory: currentMemory, loadOperand: loadOperand)
        case .i32Load16S(let loadOperand):
            try self.i32Load16S(runtime: runtime, stack: &stack, currentMemory: currentMemory, loadOperand: loadOperand)
        case .i32Load16U(let loadOperand):
            try self.i32Load16U(runtime: runtime, stack: &stack, currentMemory: currentMemory, loadOperand: loadOperand)
        case .i64Load8S(let loadOperand):
            try self.i64Load8S(runtime: runtime, stack: &stack, currentMemory: currentMemory, loadOperand: loadOperand)
        case .i64Load8U(let loadOperand):
            try self.i64Load8U(runtime: runtime, stack: &stack, currentMemory: currentMemory, loadOperand: loadOperand)
        case .i64Load16S(let loadOperand):
            try self.i64Load16S(runtime: runtime, stack: &stack, currentMemory: currentMemory, loadOperand: loadOperand)
        case .i64Load16U(let loadOperand):
            try self.i64Load16U(runtime: runtime, stack: &stack, currentMemory: currentMemory, loadOperand: loadOperand)
        case .i64Load32S(let loadOperand):
            try self.i64Load32S(runtime: runtime, stack: &stack, currentMemory: currentMemory, loadOperand: loadOperand)
        case .i64Load32U(let loadOperand):
            try self.i64Load32U(runtime: runtime, stack: &stack, currentMemory: currentMemory, loadOperand: loadOperand)
        case .i32Store(let storeOperand):
            try self.i32Store(runtime: runtime, stack: &stack, currentMemory: currentMemory, storeOperand: storeOperand)
        case .i64Store(let storeOperand):
            try self.i64Store(runtime: runtime, stack: &stack, currentMemory: currentMemory, storeOperand: storeOperand)
        case .f32Store(let storeOperand):
            try self.f32Store(runtime: runtime, stack: &stack, currentMemory: currentMemory, storeOperand: storeOperand)
        case .f64Store(let storeOperand):
            try self.f64Store(runtime: runtime, stack: &stack, currentMemory: currentMemory, storeOperand: storeOperand)
        case .i32Store8(let storeOperand):
            try self.i32Store8(runtime: runtime, stack: &stack, currentMemory: currentMemory, storeOperand: storeOperand)
        case .i32Store16(let storeOperand):
            try self.i32Store16(runtime: runtime, stack: &stack, currentMemory: currentMemory, storeOperand: storeOperand)
        case .i64Store8(let storeOperand):
            try self.i64Store8(runtime: runtime, stack: &stack, currentMemory: currentMemory, storeOperand: storeOperand)
        case .i64Store16(let storeOperand):
            try self.i64Store16(runtime: runtime, stack: &stack, currentMemory: currentMemory, storeOperand: storeOperand)
        case .i64Store32(let storeOperand):
            try self.i64Store32(runtime: runtime, stack: &stack, currentMemory: currentMemory, storeOperand: storeOperand)
