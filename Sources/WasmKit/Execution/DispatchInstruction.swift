...
extension Execution {
    ...
    mutating func doExecute(_ opcode: OpcodeID, sp: inout Sp, pc: inout Pc, md: inout Md, ms: inout Ms) throws -> CodeSlot {
        ...
        case 41: return try self.execute_i32LoadUnchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 42: return try self.execute_i64LoadUnchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 43: return try self.execute_f32LoadUnchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 44: return try self.execute_f64LoadUnchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 45: return try self.execute_i32Load8SUnchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 46: return try self.execute_i32Load8UUnchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 47: return try self.execute_i32Load16SUnchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 48: return try self.execute_i32Load16UUnchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 49: return try self.execute_i64Load8SUnchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 50: return try self.execute_i64Load8UUnchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 51: return try self.execute_i64Load16SUnchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 52: return try self.execute_i64Load16UUnchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 53: return try self.execute_i64Load32SUnchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 54: return try self.execute_i64Load32UUnchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 55: return try self.execute_i32StoreUnchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 56: return try self.execute_i64StoreUnchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 57: return try self.execute_f32StoreUnchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 58: return try self.execute_f64StoreUnchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 59: return try self.execute_i32Store8Unchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 60: return try self.execute_i32Store16Unchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 61: return try self.execute_i64Store8Unchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 62: return try self.execute_i64Store16Unchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 63: return try self.execute_i64Store32Unchecked(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 64: return self.execute_memorySize(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 65: return try self.execute_memoryGrow(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 66: return try self.execute_memoryInit(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 67: return self.execute_memoryDataDrop(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 68: return try self.execute_memoryCopy(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 69: return try self.execute_memoryFill(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 70: return self.execute_v128Const(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 71: return self.execute_i8x16Shuffle(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 72: return try self.execute_simd(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 73: return self.execute_const32(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 74: return self.execute_const64(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 75: return self.execute_i32Add(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 76: return self.execute_i64Add(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 77: return self.execute_i32Sub(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 78: return self.execute_i64Sub(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 79: return self.execute_i32Mul(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 80: return self.execute_i64Mul(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 81: return self.execute_i32And(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 82: return self.execute_i64And(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 83: return self.execute_i32Or(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 84: return self.execute_i64Or(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 85: return self.execute_i32Xor(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 86: return self.execute_i64Xor(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 87: return self.execute_i32Shl(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 88: return self.execute_i64Shl(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 89: return self.execute_i32ShrS(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 90: return self.execute_i64ShrS(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 91: return self.execute_i32ShrU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 92: return self.execute_i64ShrU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 93: return self.execute_i32Rotl(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 94: return self.execute_i64Rotl(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 95: return self.execute_i32Rotr(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 96: return self.execute_i64Rotr(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 97: return try self.execute_i32DivS(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 98: return try self.execute_i64DivS(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 99: return try self.execute_i32DivU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 100: return try self.execute_i64DivU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 101: return try self.execute_i32RemS(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 102: return try self.execute_i64RemS(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 103: return try self.execute_i32RemU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 104: return try self.execute_i64RemU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 105: return self.execute_i32Eq(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 106: return self.execute_i64Eq(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 107: return self.execute_i32Ne(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 108: return self.execute_i64Ne(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 109: return self.execute_i32LtS(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 110: return self.execute_i64LtS(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 111: return self.execute_i32LtU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 112: return self.execute_i64LtU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 113: return self.execute_i32GtS(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 114: return self.execute_i64GtS(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 115: return self.execute_i32GtU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 116: return self.execute_i64GtU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 117: return self.execute_i32LeS(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 118: return self.execute_i64LeS(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 119: return self.execute_i32LeU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 120: return self.execute_i64LeU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 121: return self.execute_i32GeS(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 122: return self.execute_i64GeS(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 123: return self.execute_i32GeU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 124: return self.execute_i64GeU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 125: return self.execute_i32Clz(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 126: return self.execute_i64Clz(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 127: return self.execute_i32Ctz(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 128: return self.execute_i64Ctz(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 129: return self.execute_i32Popcnt(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 130: return self.execute_i64Popcnt(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 131: return self.execute_i32Eqz(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 132: return self.execute_i64Eqz(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 133: return self.execute_i32WrapI64(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 134: return self.execute_i64ExtendI32S(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 135: return self.execute_i64ExtendI32U(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 136: return self.execute_i32Extend8S(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 137: return self.execute_i64Extend8S(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 138: return self.execute_i32Extend16S(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 139: return self.execute_i64Extend16S(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 140: return self.execute_i64Extend32S(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 141: return try self.execute_i32TruncF32S(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 142: return try self.execute_i32TruncF32U(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 143: return try self.execute_i32TruncSatF32S(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 144: return try self.execute_i32TruncSatF32U(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 145: return try self.execute_i32TruncF64S(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 146: return try self.execute_i32TruncF64U(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 147: return try self.execute_i32TruncSatF64S(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 148: return try self.execute_i32TruncSatF64U(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 149: return try self.execute_i64TruncF32S(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 150: return try self.execute_i64TruncF32U(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 151: return try self.execute_i64TruncSatF32S(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 152: return try self.execute_i64TruncSatF32U(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 153: return try self.execute_i64TruncF64S(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 154: return try self.execute_i64TruncF64U(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 155: return try self.execute_i64TruncSatF64S(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 156: return try self.execute_i64TruncSatF64U(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 157: return self.execute_f32ConvertI32S(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 158: return self.execute_f32ConvertI32U(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 159: return self.execute_f32ConvertI64S(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 160: return self.execute_f32ConvertI64U(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 161: return self.execute_f64ConvertI32S(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 162: return self.execute_f64ConvertI32U(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 163: return self.execute_f64ConvertI64S(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 164: return self.execute_f64ConvertI64U(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 165: return self.execute_f32ReinterpretI32(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 166: return self.execute_f64ReinterpretI64(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 167: return self.execute_i32ReinterpretF32(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 168: return self.execute_i64ReinterpretF64(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 169: return self.execute_f32Add(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 170: return self.execute_f64Add(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 171: return self.execute_f32Sub(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 172: return self.execute_f64Sub(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 173: return self.execute_f32Mul(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 174: return self.execute_f64Mul(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 175: return self.execute_f32Div(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 176: return self.execute_f64Div(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 177: return self.execute_f32Min(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 178: return self.execute_f64Min(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 179: return self.execute_f32Max(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 180: return self.execute_f64Max(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 181: return self.execute_f32CopySign(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 182: return self.execute_f64CopySign(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 183: return self.execute_f32Eq(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 184: return self.execute_f64Eq(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 185: return self.execute_f32Ne(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 186: return self.execute_f64Ne(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 187: return self.execute_f32Lt(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 188: return self.execute_f64Lt(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 189: return self.execute_f32Gt(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 190: return self.execute_f64Gt(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 191: return self.execute_f32Le(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 192: return self.execute_f64Le(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 193: return self.execute_f32Ge(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 194: return self.execute_f64Ge(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 195: return self.execute_f32Abs(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 196: return self.execute_f64Abs(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 197: return self.execute_f32Neg(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 198: return self.execute_f64Neg(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 199: return self.execute_f32Ceil(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 200: return self.execute_f64Ceil(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 201: return self.execute_f32Floor(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 202: return self.execute_f64Floor(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 203: return self.execute_f32Trunc(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 204: return self.execute_f64Trunc(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 205: return self.execute_f32Nearest(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 206: return self.execute_f64Nearest(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 207: return self.execute_f32Sqrt(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 208: return self.execute_f64Sqrt(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 209: return self.execute_f64PromoteF32(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 210: return self.execute_f32DemoteF64(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 211: return self.execute_select(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 212: return self.execute_refNull(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 213: return self.execute_refIsNull(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 214: return self.execute_refFunc(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 215: return try self.execute_tableGet(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 216: return try self.execute_tableSet(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 217: return self.execute_tableSize(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 218: return try self.execute_tableGrow(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 219: return try self.execute_tableFill(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 220: return try self.execute_tableCopy(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 221: return try self.execute_tableInit(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 222: return self.execute_tableElementDrop(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 223: return self.execute_onEnter(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 224: return self.execute_onExit(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 225: return try self.execute_breakpoint(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 226: return try self.execute_i32AtomicLoad(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 227: return try self.execute_i64AtomicLoad(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 228: return try self.execute_i32AtomicLoad8U(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 229: return try self.execute_i32AtomicLoad16U(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 230: return try self.execute_i64AtomicLoad8U(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 231: return try self.execute_i64AtomicLoad16U(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 232: return try self.execute_i64AtomicLoad32U(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 233: return try self.execute_i32AtomicStore(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 234: return try self.execute_i64AtomicStore(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 235: return try self.execute_i32AtomicStore8(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 236: return try self.execute_i32AtomicStore16(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 237: return try self.execute_i64AtomicStore8(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 238: return try self.execute_i64AtomicStore16(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 239: return try self.execute_i64AtomicStore32(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 240: return try self.execute_i32AtomicRmwAdd(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 241: return try self.execute_i64AtomicRmwAdd(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 242: return try self.execute_i32AtomicRmwSub(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 243: return try self.execute_i64AtomicRmwSub(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 244: return try self.execute_i32AtomicRmwAnd(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 245: return try self.execute_i64AtomicRmwAnd(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 246: return try self.execute_i32AtomicRmwOr(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 247: return try self.execute_i64AtomicRmwOr(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 248: return try self.execute_i32AtomicRmwXor(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 249: return try self.execute_i64AtomicRmwXor(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 250: return try self.execute_i32AtomicRmwXchg(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 251: return try self.execute_i64AtomicRmwXchg(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 252: return try self.execute_i32AtomicRmw8AddU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 253: return try self.execute_i64AtomicRmw8AddU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 254: return try self.execute_i32AtomicRmw8SubU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 255: return try self.execute_i64AtomicRmw8SubU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 256: return try self.execute_i32AtomicRmw8AndU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 257: return try self.execute_i64AtomicRmw8AndU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 258: return try self.execute_i32AtomicRmw8OrU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 259: return try self.execute_i64AtomicRmw8OrU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 260: return try self.execute_i32AtomicRmw8XorU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 261: return try self.execute_i64AtomicRmw8XorU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 262: return try self.execute_i32AtomicRmw8XchgU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 263: return try self.execute_i64AtomicRmw8XchgU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 264: return try self.execute_i32AtomicRmw16AddU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 265: return try self.execute_i64AtomicRmw16AddU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 266: return try self.execute_i32AtomicRmw16SubU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 267: return try self.execute_i64AtomicRmw16SubU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 268: return try self.execute_i32AtomicRmw16AndU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 269: return try self.execute_i64AtomicRmw16AndU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 270: return try self.execute_i32AtomicRmw16OrU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 271: return try self.execute_i64AtomicRmw16OrU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 272: return try self.execute_i32AtomicRmw16XorU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 273: return try self.execute_i64AtomicRmw16XorU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 274: return try self.execute_i32AtomicRmw16XchgU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 275: return try self.execute_i64AtomicRmw16XchgU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 276: return try self.execute_i64AtomicRmw32AddU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 277: return try self.execute_i64AtomicRmw32SubU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 278: return try self.execute_i64AtomicRmw32AndU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 279: return try self.execute_i64AtomicRmw32OrU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 280: return try self.execute_i64AtomicRmw32XorU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 281: return try self.execute_i64AtomicRmw32XchgU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 282: return try self.execute_i32AtomicRmwCmpxchg(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 283: return try self.execute_i64AtomicRmwCmpxchg(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 284: return try self.execute_i32AtomicRmw8CmpxchgU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 285: return try self.execute_i32AtomicRmw16CmpxchgU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 286: return try self.execute_i64AtomicRmw8CmpxchgU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 287: return try self.execute_i64AtomicRmw16CmpxchgU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 288: return try self.execute_i64AtomicRmw32CmpxchgU(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 289: return try self.execute_memoryAtomicWait32(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 290: return try self.execute_memoryAtomicWait64(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 291: return try self.execute_memoryAtomicNotify(sp: &sp, pc: &pc, md: &md, ms: &ms)
        case 292: return self.execute_atomicFence(sp: &sp, pc: &pc, md: &md, ms: &ms)
...
extension Execution {
    ...
    @_silgen_name("wasmkit_execute_i32LoadUnchecked") @inline(__always)
    mutating func execute_i32LoadUnchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.LoadOperand.load(from: &pc.pointee)
        try memoryLoadUnchecked(sp: sp.pointee, md: md.pointee, loadOperand: immediate, loadAs: UInt32.self, castToValue: { .i32($0) })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_i64LoadUnchecked") @inline(__always)
    mutating func execute_i64LoadUnchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.LoadOperand.load(from: &pc.pointee)
        try memoryLoadUnchecked(sp: sp.pointee, md: md.pointee, loadOperand: immediate, loadAs: UInt64.self, castToValue: { .i64($0) })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_f32LoadUnchecked") @inline(__always)
    mutating func execute_f32LoadUnchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.LoadOperand.load(from: &pc.pointee)
        try memoryLoadUnchecked(sp: sp.pointee, md: md.pointee, loadOperand: immediate, loadAs: UInt32.self, castToValue: { .rawF32($0) })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_f64LoadUnchecked") @inline(__always)
    mutating func execute_f64LoadUnchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.LoadOperand.load(from: &pc.pointee)
        try memoryLoadUnchecked(sp: sp.pointee, md: md.pointee, loadOperand: immediate, loadAs: UInt64.self, castToValue: { .rawF64($0) })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_i32Load8SUnchecked") @inline(__always)
    mutating func execute_i32Load8SUnchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.LoadOperand.load(from: &pc.pointee)
        try memoryLoadUnchecked(sp: sp.pointee, md: md.pointee, loadOperand: immediate, loadAs: Int8.self, castToValue: { .init(signed: Int32($0)) })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_i32Load8UUnchecked") @inline(__always)
    mutating func execute_i32Load8UUnchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.LoadOperand.load(from: &pc.pointee)
        try memoryLoadUnchecked(sp: sp.pointee, md: md.pointee, loadOperand: immediate, loadAs: UInt8.self, castToValue: { .i32(UInt32($0)) })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_i32Load16SUnchecked") @inline(__always)
    mutating func execute_i32Load16SUnchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.LoadOperand.load(from: &pc.pointee)
        try memoryLoadUnchecked(sp: sp.pointee, md: md.pointee, loadOperand: immediate, loadAs: Int16.self, castToValue: { .init(signed: Int32($0)) })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_i32Load16UUnchecked") @inline(__always)
    mutating func execute_i32Load16UUnchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.LoadOperand.load(from: &pc.pointee)
        try memoryLoadUnchecked(sp: sp.pointee, md: md.pointee, loadOperand: immediate, loadAs: UInt16.self, castToValue: { .i32(UInt32($0)) })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_i64Load8SUnchecked") @inline(__always)
    mutating func execute_i64Load8SUnchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.LoadOperand.load(from: &pc.pointee)
        try memoryLoadUnchecked(sp: sp.pointee, md: md.pointee, loadOperand: immediate, loadAs: Int8.self, castToValue: { .init(signed: Int64($0)) })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_i64Load8UUnchecked") @inline(__always)
    mutating func execute_i64Load8UUnchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.LoadOperand.load(from: &pc.pointee)
        try memoryLoadUnchecked(sp: sp.pointee, md: md.pointee, loadOperand: immediate, loadAs: UInt8.self, castToValue: { .i64(UInt64($0)) })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_i64Load16SUnchecked") @inline(__always)
    mutating func execute_i64Load16SUnchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.LoadOperand.load(from: &pc.pointee)
        try memoryLoadUnchecked(sp: sp.pointee, md: md.pointee, loadOperand: immediate, loadAs: Int16.self, castToValue: { .init(signed: Int64($0)) })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_i64Load16UUnchecked") @inline(__always)
    mutating func execute_i64Load16UUnchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.LoadOperand.load(from: &pc.pointee)
        try memoryLoadUnchecked(sp: sp.pointee, md: md.pointee, loadOperand: immediate, loadAs: UInt16.self, castToValue: { .i64(UInt64($0)) })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_i64Load32SUnchecked") @inline(__always)
    mutating func execute_i64Load32SUnchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.LoadOperand.load(from: &pc.pointee)
        try memoryLoadUnchecked(sp: sp.pointee, md: md.pointee, loadOperand: immediate, loadAs: Int32.self, castToValue: { .init(signed: Int64($0)) })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_i64Load32UUnchecked") @inline(__always)
    mutating func execute_i64Load32UUnchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.LoadOperand.load(from: &pc.pointee)
        try memoryLoadUnchecked(sp: sp.pointee, md: md.pointee, loadOperand: immediate, loadAs: UInt32.self, castToValue: { .i64(UInt64($0)) })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_i32StoreUnchecked") @inline(__always)
    mutating func execute_i32StoreUnchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.StoreOperand.load(from: &pc.pointee)
        try memoryStoreUnchecked(sp: sp.pointee, md: md.pointee, storeOperand: immediate, castFromValue: { $0.i32 })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_i64StoreUnchecked") @inline(__always)
    mutating func execute_i64StoreUnchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.StoreOperand.load(from: &pc.pointee)
        try memoryStoreUnchecked(sp: sp.pointee, md: md.pointee, storeOperand: immediate, castFromValue: { $0.i64 })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_f32StoreUnchecked") @inline(__always)
    mutating func execute_f32StoreUnchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.StoreOperand.load(from: &pc.pointee)
        try memoryStoreUnchecked(sp: sp.pointee, md: md.pointee, storeOperand: immediate, castFromValue: { $0.rawF32 })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_f64StoreUnchecked") @inline(__always)
    mutating func execute_f64StoreUnchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.StoreOperand.load(from: &pc.pointee)
        try memoryStoreUnchecked(sp: sp.pointee, md: md.pointee, storeOperand: immediate, castFromValue: { $0.rawF64 })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_i32Store8Unchecked") @inline(__always)
    mutating func execute_i32Store8Unchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.StoreOperand.load(from: &pc.pointee)
        try memoryStoreUnchecked(sp: sp.pointee, md: md.pointee, storeOperand: immediate, castFromValue: { UInt8(truncatingIfNeeded: $0.i32) })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_i32Store16Unchecked") @inline(__always)
    mutating func execute_i32Store16Unchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.StoreOperand.load(from: &pc.pointee)
        try memoryStoreUnchecked(sp: sp.pointee, md: md.pointee, storeOperand: immediate, castFromValue: { UInt16(truncatingIfNeeded: $0.i32) })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_i64Store8Unchecked") @inline(__always)
    mutating func execute_i64Store8Unchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.StoreOperand.load(from: &pc.pointee)
        try memoryStoreUnchecked(sp: sp.pointee, md: md.pointee, storeOperand: immediate, castFromValue: { UInt8(truncatingIfNeeded: $0.i64) })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_i64Store16Unchecked") @inline(__always)
    mutating func execute_i64Store16Unchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.StoreOperand.load(from: &pc.pointee)
        try memoryStoreUnchecked(sp: sp.pointee, md: md.pointee, storeOperand: immediate, castFromValue: { UInt16(truncatingIfNeeded: $0.i64) })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
    @_silgen_name("wasmkit_execute_i64Store32Unchecked") @inline(__always)
    mutating func execute_i64Store32Unchecked(sp: UnsafeMutablePointer<Sp>, pc: UnsafeMutablePointer<Pc>, md: UnsafeMutablePointer<Md>, ms: UnsafeMutablePointer<Ms>) throws -> CodeSlot {
        let immediate = Instruction.StoreOperand.load(from: &pc.pointee)
        try memoryStoreUnchecked(sp: sp.pointee, md: md.pointee, storeOperand: immediate, castFromValue: { UInt32(truncatingIfNeeded: $0.i64) })
        let next = pc.pointee.pointee
        pc.pointee = pc.pointee.advanced(by: 1)
        return next
    }
