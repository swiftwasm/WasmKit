...
extension Execution {
    ...
    mutating func memorySize(sp: Sp, immediate: Instruction.MemorySizeOperand) {
        ...
        let pageCount = memory.withValue { $0.data.count } / MemoryEntity.pageSize
        let value: Value = memory.withValue { $0.limit.isMemory64 } ? .i64(UInt64(pageCount)) : .i32(UInt32(pageCount))
