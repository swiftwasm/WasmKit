...
extension Execution {
    ...
    /// `bl` anywhere in it -- and switching `md`/`ms` needs one, for
    /// `wasmkit_trap_guard_set_current_memory` -- would cost a stack frame's
    /// worth of prologue and epilogue on *every* return.
    @inline(__always)
