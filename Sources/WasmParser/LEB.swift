...
func decodeLEB128<IntType, Stream>(
    stream: Stream
) throws(WasmKitError) -> IntType where IntType: FixedWidthInteger, IntType: UnsignedInteger, Stream: ByteStream {
    ...
    while true {
        ...
        if nextShift >= IntType.bitWidth, (byte >> (UInt(IntType.bitWidth) - shift)) != 0 {
            throw .leb(LEBError.integerRepresentationTooLong, offset: stream.currentIndex)
        }
        result |= slice << shift
        shift = nextShift
...
) throws(WasmKitError) -> IntType where IntType: FixedWidthInteger, IntType: RawSignedInteger, Stream: ByteStream {
    let firstByte = try stream.consumeAny()
    ...
    repeat {
        ...
        if shift > (bitWidth - 7) {
            ...
            if signAndDiscardingBits != 0 && signAndDiscardingBits != -1 {
                throw .leb(LEBError.overflow, offset: stream.currentIndex)
            }
