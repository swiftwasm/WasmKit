...
@inlinable
func decodeLEB128<IntType>(
    stream: inout BufferSlice.Cursor
) throws -> IntType where IntType: FixedWidthInteger, IntType: UnsignedInteger {
    let firstByte = try stream.next()
    var result: IntType = IntType(firstByte & 0b0111_1111)
    if _fastPath(firstByte & 0b1000_0000 == 0) {
        return result
    }
...
func decodeLEB128<IntType>(
    stream: inout BufferSlice.Cursor
) throws -> IntType where IntType: FixedWidthInteger, IntType: RawSignedInteger {
    let firstByte = try stream.next()
    var result = IntType.Unsigned(firstByte & 0b0111_1111)
    if _fastPath(firstByte & 0b1000_0000 == 0) {
        // Interpret Int${Self.bitWidth-1} as Int${Self.bitWidth}
