// File metadata and timestamp types for the WASI platform layer.
    ...
    import WASILibc
        ...
        struct FileTime {
            ...
            // Glibc and Bionic define `UTIME_NOW`/`UTIME_OMIT` as complex macros
            // that ClangImporter can't import; use the (identical) values directly.
            #if canImport(Glibc) || canImport(Android)
                private static var utimeNow: Int { (1 << 30) - 1 }
                private static var utimeOmit: Int { (1 << 30) - 2 }
            #else
                private static var utimeNow: Int { Int(UTIME_NOW) }
                private static var utimeOmit: Int { Int(UTIME_OMIT) }
            #endif

            static var now: FileTime {
                FileTime(rawValue: timespec(tv_sec: 0, tv_nsec: Self.utimeNow))
            }
            static var omit: FileTime {
                FileTime(rawValue: timespec(tv_sec: 0, tv_nsec: Self.utimeOmit))
            }
        }
    #endif
