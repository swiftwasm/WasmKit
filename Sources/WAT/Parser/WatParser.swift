...
struct WatParser {
    ...
    struct FunctionDecl: NamedFieldDecl, ImportableModuleFieldDecl {
        ...
        func parse<V: InstructionVisitor>(
            ...
            guard try parser.parser.isEndOfParen() else {
                throw WasmKitError("unexpected token", location: parser.parser.lexer.location())
            }
            return typeIndex
        }
    }
    ...
    mutating func next() throws(WasmKitError) -> ModuleField? {
        ...
        case "import":
            ...
            if try parser.takeParenBlockStart("func") {
                ...
                throw WasmKitError("unexpected token", location: parser.lexer.location())
            }
        ...
        case "table":
            ...
            if let refType = try takeRefType() {
                guard try parser.takeParenBlockStart("elem") else {
                    throw WasmKitError("expected elem", location: parser.lexer.location())
                }
                var numberOfItems: UInt64 = 0
        ...
        case "data":
            ...
            try parser.expect(.rightParen)
        default:
            throw WasmKitError("unexpected module field \(keyword)", location: location)
        }
    ...
    mutating func valueType() throws(WasmKitError) -> UnresolvedType<ValueType> {
        if try parser.takeKeyword("i32") {
            ...
            throw WasmKitError("expected value type", location: parser.lexer.location())
        }
    }

    mutating func refType() throws(WasmKitError) -> UnresolvedType<ReferenceType> {
        guard let refType = try takeRefType() else {
            throw WasmKitError("expected reference type", location: parser.lexer.location())
        }
        return refType
    }
    ...
    mutating func heapType() throws(WasmKitError) -> UnresolvedType<HeapType> {
        ...
        throw WasmKitError("expected heap type", location: parser.lexer.location())
    }
}
