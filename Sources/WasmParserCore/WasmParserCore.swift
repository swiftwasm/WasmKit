...
package let WASM_MAGIC: [UInt8] = [0x00, 0x61, 0x73, 0x6D]
...
extension ByteStream {
    package func parseName() throws(WasmParserError) -> String {
        ...
        #if $Embedded
        // Embedded Swift fast path: create String directly from UTF-8 bytes.
        // Avoids the character-by-character append loop which uses Unicode scalar
        // handling and pulls in NFC normalization machinery (~31 KB of tables).
        guard let name = String(validating: bytes, as: UTF8.self) else {
            throw WasmParserError(message: .invalidUTF8(bytes), offset: currentIndex)
        }
        return name
        #else
