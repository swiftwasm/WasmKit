...
extension WASIBridgeToHost {
    ...
    // MARK: - wasi-threads support
        ...
        /// Set up wasi-threads support for `module`, defining `thread-spawn` in the `"wasi"` namespace.
        ///
        /// Call ``link(to:store:)`` first: this does not define the `wasi_snapshot_preview1` functions.
        ///
        /// - Returns: A ``ThreadGroup`` whose ``ThreadGroup/joinAllThreads()`` the caller must call
        ...
        package func linkThreads(
            ...
            // A trap or proc_exit in any child must terminate the parent too.
