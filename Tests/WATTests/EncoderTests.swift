...
struct EncoderTests {
    ...
    // MARK: - Test Cases
        ...
        func annotationProposalSpectest(wastFile: URL) throws {
            var stats = CompatibilityTestStats()
            try TestSupport.withTemporaryDirectory { tempDir, shouldRetain in
                let watModules: [ModuleDirective]
                do {
                ...
                let wastContent = try Data(contentsOf: wastFile)
    ...
    func encodeNameSectionAllSubsections() throws {
        ...
        let nameParser = NameSectionParser(
            stream: StaticByteStream(bytes: nameBytes ?? [])
        )
    ...
    func nameAnnotationWast() throws {
        ...
        func customAnnotationWast() throws {
            ...
            for watModule in watModules {
                if case .text(let wat) = watModule.source {
                    _ = try wat.encode()
                    encodedCount += 1
                }
            }
    ...
    private func findCustomSections(in bytes: [UInt8]) throws -> [(name: String, content: ArraySlice<UInt8>)] {
        var parser = WasmParser.Parser(bytes: bytes)
        var sections: [(name: String, content: ArraySlice<UInt8>)] = []
        while let payload = try parser.parseNext() {
            if case .customSection(let section) = payload {
                sections.append((name: section.name, content: section.bytes))
            }
        }
        return sections
    }
    ...
    func customAnnotationBasicEncoding() throws {
        ...
        #expect(sections[0].content.elementsEqual("hello".utf8))
    }
    ...
    func customAnnotationEmptyName() throws {
        ...
        #expect(sections[0].content.elementsEqual("data".utf8))
    }
    ...
    func customAnnotationMultipleStrings() throws {
        ...
        let sections = try findCustomSections(in: bytes)
        #expect(sections.count == 1)
        #expect(sections[0].content.elementsEqual("abcdef".utf8))
    }
    ...
    func customAnnotationDuplicateNames() throws {
        ...
        #expect(sections[0].content.elementsEqual("a".utf8))
        #expect(sections[1].content.elementsEqual("b".utf8))
        #expect(sections[2].content.elementsEqual("c".utf8))
    }
