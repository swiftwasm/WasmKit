...
/// reference output.
@Suite struct WASIp2ParseTests {

    private static let vendorBase: String = {
    ...
    private func parsePackage(_ name: String) throws -> PackageUnit {
        try PackageUnit.parse(
            directory: "\(Self.vendorBase)/\(name)/wit",
            loader: loader
        )
    }
    ...
    private func getReferenceOutput(_ name: String, deps: [String]) throws -> String {
        if deps.isEmpty {
            ...
            return try componentWit(packageDirectory: "\(Self.vendorBase)/\(name)/wit")
        } else {
            ...
            let srcDir = URL(fileURLWithPath: "\(Self.vendorBase)/\(name)/wit")
            ...
            for dep in deps {
                let depSrc = URL(fileURLWithPath: "\(Self.vendorBase)/\(dep)/wit")
