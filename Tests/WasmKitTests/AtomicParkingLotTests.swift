// The parking lot's blocking implementation exists only with the MultiThread trait.
#if MultiThread
    ...
    // (see AtomicParkingLot.swift), and `swift test` there must not compile this suite.
        ...
        @Suite struct AtomicParkingLotTests {
                ...
                @Test func timedOutWaiterClaimedByNotifyReportsWoken() {
                    ...
                    Thread.detachNewThread {
                        let result = lot.parkConditionally(
                            address: address,
                            validate: { true },
                            deadline: ParkingDeadline(afterNanoseconds: 0),  // relative timeout 0: `wait` reads `woken` immediately
                            beforeSleep: {
                                registered.signal()
                                proceedWaiter.wait()
                            }
            ...
            @Test func notifyCountMatchesObservedWakeupsUnderContention() {
                ...
                let waitTimeoutNanoseconds: Int64 = 200_000
                    ...
                    Thread.detachNewThread {
                        ...
                        while ContinuousClock.now < stopAt {
                            switch lot.parkConditionally(
                                address: address,
                                validate: { true },
                                deadline: ParkingDeadline(afterNanoseconds: waitTimeoutNanoseconds)
                            ) {
                            case .woken: woken += 1
